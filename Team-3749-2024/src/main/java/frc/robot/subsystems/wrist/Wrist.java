...
public class Wrist extends SubsystemBase {
  ...
  private ProfiledPIDController wristController = new ProfiledPIDController(
    Constants.WristConstants.PID.kP,
    Constants.WristConstants.PID.kI,
    Constants.WristConstants.PID.kD,
    Constants.WristConstants.trapezoidConstraint
  );

  private ArmFeedforward wristFF = new ArmFeedforward(
    Constants.WristConstants.simkS,
    Constants.WristConstants.simkG,
  ...
  private MechanismRoot2d mechanismArmPivot = mechanism.getRoot(
    "mechanism arm pivot",
    1,
    0.5
  );
  private MechanismLigament2d mechanismArm = mechanismArmPivot.append(
    new MechanismLigament2d("mechanism arm", .93, 0)
  );

  private ShuffleData<Double> positionLog = new ShuffleData<Double>(
    this.getName(),
    "position",
    0.0
  );
  ...
  private ShuffleData<Double> voltageLog = new ShuffleData<Double>(
    this.getName(),
    "voltage",
    0.0
  );
  private ShuffleData<Double> currentLog = new ShuffleData<Double>(
    this.getName(),
    "current",
    0.0
  );

  private ShuffleData<Double> goalLog = new ShuffleData<Double>(
    this.getName(),
    "goal",
    0.0
  );
  ...
  private ShuffleData<Double> errorPositionLog = new ShuffleData<Double>(
    this.getName(),
    "error position",
    0.0
  );
  private ShuffleData<Double> errorVelocityLog = new ShuffleData<Double>(
    this.getName(),
    "error velocity",
    0.0
  );
  ...
  private ShuffleData<Double> kPData = new ShuffleData(
    this.getName(),
    "kpdata",
    0.0
  );
  private ShuffleData<Double> kVData = new ShuffleData(
    this.getName(),
    "kVdata",
    0.0
  );
  ...
  private ShuffleData<Double> kGData = new ShuffleData<Double>(
    "wrist",
    "kGData",
    0.0
  );
  ...
  public double calculateGravityFeedForward(
    double wristPositionRad,
    double armPositionRad
  ) {
    return (
      WristConstants.kYIntercept +
      WristConstants.kBar *
      wristPositionRad +
      WristConstants.kBarSquared *
      Math.pow(wristPositionRad, 2) +
      WristConstants.kBarCubed *
      Math.pow(wristPositionRad, 3) +
      WristConstants.kArm *
      armPositionRad +
      WristConstants.kArmSquared *
      Math.pow(armPositionRad, 2) +
      WristConstants.kBarArm *
      wristPositionRad *
      armPositionRad +
      WristConstants.kBarSquaredArm *
      Math.pow(wristPositionRad, 2) *
      armPositionRad +
      WristConstants.kBarCubedArm *
      Math.pow(wristPositionRad, 3) *
      armPositionRad +
      WristConstants.kBarArmSquared *
      wristPositionRad *
      Math.pow(armPositionRad, 2) +
      WristConstants.kBarSquaredArmSquared *
      Math.pow(wristPositionRad, 2) *
      Math.pow(armPositionRad, 2) +
      WristConstants.kBarCubedArmSquared *
      Math.pow(wristPositionRad, 3) *
      Math.pow(armPositionRad, 2)
    );
  }
